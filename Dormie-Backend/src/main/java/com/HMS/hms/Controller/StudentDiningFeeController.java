...
public class StudentDiningFeeController {
    ...
    // Get student dining fee payment information by user ID
    @GetMapping("/payment/user/{userId}")
    @PreAuthorize("isAuthenticated()")
    public ResponseEntity<List<StudentDiningFeePaymentDTO>> getStudentDiningFeePaymentsByUserId(@PathVariable Long userId) {
        try {
            List<StudentDiningFeePaymentDTO> payments = studentDiningFeeService.getDiningFeePaymentsByUserId(userId);
