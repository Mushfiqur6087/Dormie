...
// --- FIX END ---
  ...
  const IMAGE_BASE_URL = `http://172.187.160.142:8080/uploads/`;
  ...
  const fetchApplicationDetails = useCallback(async () => {
    ...
    try {
      const response = await fetch(`http://172.187.160.142:8080/api/applications/${id}`, {
        method: 'GET',
        headers: {
  ...
  const handleApplicationAction = useCallback(async (actionType) => {
    ...
    try {
      const response = await fetch(`http://172.187.160.142:8080/api/applications/${id}/${actionType}`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${jwtToken}`,
          'Content-Type': 'application/json',
