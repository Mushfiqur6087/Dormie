...
export default function AdminStudentBatchUpload() {
    ...
    reader.onload = async (e) => {
      try {
        ...
        for (const [index, row] of jsonRows.entries()) {
          ...
          // Use the exact column headers: Student ID, Name, Email, Department, Batch
          const studentIdFromExcel = row["Student ID"]
          const nameFromExcel = row["Name"]
          const emailFromExcel = row["Email"]
          const departmentFromExcel = row["Department"]
          const batchFromExcel = row["Batch"]

          const signupData = {
            username: nameFromExcel || emailFromExcel.split('@')[0], // Use name or email prefix as username
            email: emailFromExcel,
            password: COMMON_PASSWORD, // Hardcoded common password
            studentId: parseInt(studentIdFromExcel), // Ensure it's a number
            department: departmentFromExcel,
            batch: parseInt(batchFromExcel), // Convert to integer
          ...
          if (!signupData.username || String(signupData.username).trim() === "") {
            rowFailed = true
            rowMessage = "Missing or invalid Username"
          } else if (!signupData.email || String(signupData.email).trim() === "") {
            rowFailed = true
            rowMessage = "Missing or empty Email"
          } else if (!signupData.studentId || isNaN(Number(signupData.studentId))) {
            // Check if studentId is not a number
            rowFailed = true
            rowMessage = "Missing or invalid Student ID (must be a number)"
          } else if (!signupData.department || String(signupData.department).trim() === "") {
            rowFailed = true
            rowMessage = "Missing or empty Department"
          } else if (!signupData.batch || isNaN(Number(signupData.batch))) {
            rowFailed = true
            rowMessage = "Missing or invalid Batch (must be a number)"
          }
          ...
          try {
            ...
            if (response.ok) {
              ...
              currentResults.push({
                row: rowIndex,
                status: "Success",
                message: responseData.message || "Student user created successfully!",
                data: signupData,
              })
            } else {
              failCount++
  ...
  const downloadTemplate = () => {
    const templateData = [
      { "Student ID": "2005094", Name: "Golam", Email: "2005094@cse.buet.ac.bd", Department: "ChE", Batch: 20 },
      { "Student ID": "2005108", Name: "Musfiqur", Email: "2005108@cse.buet.ac.bd", Department: "CSE", Batch: 24 },
      { "Student ID": "2005110", Name: "Redwan", Email: "2005110@cse.buet.ac.bd", Department: "EEE", Batch: 21 },
    ]
    ...
    <div className="space-y-8">
      ...
      <div className="bg-gradient-to-r from-red-600 to-red-700 dark:from-red-700 dark:to-red-800 rounded-xl shadow-lg p-8 text-white">
        <div className="flex items-center space-x-4">
          ...
          <div>
            <h1 className="text-3xl font-bold mb-2">Batch Student Upload</h1>
            <p className="text-red-100 text-lg">Upload multiple student accounts from Excel file</p>
          </div>
        </div>
      </div>
                ...
                <table className="min-w-full text-xs">
                  <thead>
                    <tr className="border-b border-gray-300 dark:border-gray-600">
                      <th className="text-left py-2 px-1 font-medium text-gray-700 dark:text-gray-300">Student ID</th>
                      <th className="text-left py-2 px-1 font-medium text-gray-700 dark:text-gray-300">Name</th>
                      <th className="text-left py-2 px-1 font-medium text-gray-700 dark:text-gray-300 min-w-48">Email</th>
                      <th className="text-left py-2 px-1 font-medium text-gray-700 dark:text-gray-300">Department</th>
                      <th className="text-left py-2 px-1 font-medium text-gray-700 dark:text-gray-300">Batch</th>
                    </tr>
                  </thead>
                  <tbody className="space-y-1">
                    <tr>
                      <td className="py-1 px-1 text-gray-600 dark:text-gray-400">2005094</td>
                      <td className="py-1 px-1 text-gray-600 dark:text-gray-400">Golam</td>
                      <td className="py-1 px-1 text-gray-600 dark:text-gray-400 break-all">2005094@cse.buet.ac.bd</td>
                      <td className="py-1 px-1 text-gray-600 dark:text-gray-400">ChE</td>
                      <td className="py-1 px-1 text-gray-600 dark:text-gray-400">20</td>
                    </tr>
                    <tr>
                      <td className="py-1 px-1 text-gray-600 dark:text-gray-400">2005108</td>
                      <td className="py-1 px-1 text-gray-600 dark:text-gray-400">Musfiqur</td>
                      <td className="py-1 px-1 text-gray-600 dark:text-gray-400 break-all">2005108@cse.buet.ac.bd</td>
                      <td className="py-1 px-1 text-gray-600 dark:text-gray-400">CSE</td>
                      <td className="py-1 px-1 text-gray-600 dark:text-gray-400">24</td>
                    </tr>
                  </tbody>
                </table>
              </div>
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-3 text-sm text-gray-600 dark:text-gray-400">
              <div className="flex items-start space-x-2">
                ...
                <span>
                  All students will be created with default password:{" "}
                  <code className="bg-gray-200 dark:bg-gray-600 px-1 rounded">12345678</code>
                </span>
              </div>
              <div className="flex items-start space-x-2">
                <div className="w-2 h-2 bg-red-500 rounded-full mt-2"></div>
                <span>Student ID must be numeric values only</span>
              </div>
              <div className="flex items-start space-x-2">
                <div className="w-2 h-2 bg-red-500 rounded-full mt-2"></div>
              ...
              <div className="flex items-start space-x-2">
                <div className="w-2 h-2 bg-red-500 rounded-full mt-2"></div>
                <span>Department must be provided (e.g., CSE, EEE, ME, etc.)</span>
              </div>
              <div className="flex items-start space-x-2">
                <div className="w-2 h-2 bg-red-500 rounded-full mt-2"></div>
                <span>Batch must be numeric (e.g., 20, 21, 22, etc.)</span>
              </div>
            </div>
          </div>
        </div>
          ...
          >
            {processing ? (
              ...
              <>
                ...
                <span>Upload & Create Students</span>
              </>
            )}
          </button>
        </div>
                    ...
                    <div>
                      ...
                      {res.data && (
                        <div className="mt-2 text-xs text-gray-600 dark:text-gray-400 space-y-1">
                          ...
                          <p>Department: {res.data.department}</p>
                          <p>Batch: {res.data.batch}</p>
                        </div>
                      )}
                    </div>
                  </div>
